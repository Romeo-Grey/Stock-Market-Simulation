...
def MomentumStrategy(sp500_data, lookback = 5):
    returnlength = len(sp500_data['Returns'].tolist())
    sp500_hourly_data = yf.download("SPY", period=f"{returnlength + (lookback*5)}d", interval="1d")
    close_PricesMomentum = sp500_hourly_data['Close']
    close_Price = sp500_data['Close']
